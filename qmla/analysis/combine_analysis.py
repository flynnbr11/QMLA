...
def combine_analysis_plots(
    results_directory, output_file_name, variables, include_all_pngs=False
):
    import re
    ...
    class PDF(FPDF):
        ...
        def header(self):
            ...
            self.cell(80)
            # Title
            analysis_title = str("QMD Analysis [{}]".format(self.run_directory))
    ...
    output_desc = str(run_dir.split("/")[0] + "__" + run_dir.split("/")[1])
    if not results_directory.endswith("/"):
        results_directory += "/"

    pdf.set_descriptors(run_directory=run_dir)
    pdf.alias_nb_pages()
    pdf.add_page()
    ...
    for k in variables.keys():
        ...
        pdf.write(8, str(str(k) + " : "))
        pdf.set_text_color(r=0, g=0, b=0)
        pdf.write(8, str(str(variables[k]) + "\n"))
    ...
    if include_all_pngs:
        ...
        for new_img in image_list:
            pdf.add_page()
            pdf.image(new_img, w=200)

    output_file_name = str(output_desc + "_" + output_file_name)
