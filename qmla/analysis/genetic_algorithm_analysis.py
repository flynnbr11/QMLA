...
def fitness_comparison(results_by_fscore, ax=None, save_directory=None):
    ...
    if save_directory is not None:
        plt.savefig(os.path.join(save_directory, "genetic_fitness.png"))
...
def elo_rating_by_fscore(results_by_fscore, ax=None, save_directory=None):
    ...
    if save_directory is not None:
        plt.savefig(os.path.join(save_directory, "genetic_elo_ratings.png"))
...
def log_likelihood_by_fscore(results_by_fscore, ax=None, save_directory=None):
    ...
    if save_directory is not None:
        plt.savefig(os.path.join(save_directory, "genetic_log_likelihood.png"))
...
def genetic_alg_num_models(results_by_fscore, ax=None, save_directory=None):
    ...
    if save_directory is not None:
        plt.savefig(os.path.join(save_directory, "genetic_num_mods.png"))
...
def genetic_alg_fitness_plots(
    ...
    if save_directory is not None:
        plt.savefig(os.path.join(save_directory, "genetic_alg.png"))
...
def model_generation_probability(
    combined_results, unique_chromosomes, save_directory=None
):
    num_terms = unique_chromosomes.num_terms.unique()[0]
    unique_chromosomes["full_chromosome"] = [
        map_to_full_chromosome(c, num_terms) for c in unique_chromosomes.chromosome
    ]
    num_experiments = combined_results["NumExperiments"][0]
    num_particles = combined_results["NumParticles"][0]
    true_chromosome = map_to_full_chromosome(
        unique_chromosomes.true_chromosome[0], num_terms=num_terms
    )
    ...
    for i in range(num_trials):
        ...
        num_samples = abs(
            int(
                np.random.normal(
                    avg_num_mods_per_instance, std_dev_num_mods_per_instance
                )
            )
        )
        try:
            ...
            print(
                "Failed to draw {} samples from {} dist".format(
    ...
    random_width_array_upper = [
        random_sampling_prob + random_sampling_width
    ] * num_models
    random_width_array_lower = [
        random_sampling_prob - random_sampling_width
    ] * num_models
    ...
    unique_numeric_chromosome_list = list(
        unique_chromosomes.numeric_chromosome.unique()
    )
    f_scores = list(unique_chromosomes.f_score)

    counts = [numeric_chromosomes.count(a) for a in unique_numeric_chromosome_list]
    counts = [
        c / num_runs for c in counts
    ]  # so this reflects 'probability' of being generated
    ...
    for mod in all_models:
        ...
        colours[mod] = c
        f_score = f_score_from_chromosome_string(
            chromosome=chromosome, target_chromosome=true_chromosome
        )
    ...
    ax.scatter(all_models, array_counts, edgecolors=colours, facecolor="none")
    ...
    ax.set_yticklabels(labels=probs_to_label, fontdict={"fontsize": label_fontsize})
    ax.axvline(int(true_chromosome, 2), c=cmap(0.0), ls=":")
    ax.axhline(
    ...
    ax2.set_yticklabels(range(num_runs + 1), fontdict={"fontsize": label_fontsize})
    if num_runs > 9:
        ax2.set_yticks(
            [0, 0.5, 1.0],
        )
    ...
    ax.axhline(random_sampling_prob, label="Prob random generation", c="black", ls="--")
    ax.set_ylim(-0.1, 1.2)
    ...
    ax2.set_title(
        "Probability of model generation. {} experiments; {} particles; {} runs".format(
            num_experiments, num_particles, num_runs
        ),
        fontsize=label_fontsize,
    )
    if save_directory is not None:
...
def correlation_fitness_f_score(combined_datasets_directory, save_directory):
    try:
    ...
    fig.savefig(os.path.join(save_directory, "fitness_f_score_correlations.png"))
    fig, ax = plt.subplots(figsize=(20, 8))
    sns.boxplot(
        y="Covariance",
        x="Method",
    ...
    fig.savefig(os.path.join(save_directory, "fitness_f_score_covariance.png"))
