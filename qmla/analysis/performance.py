...
def cross_instance_bayes_factor_heatmap(combined_datasets_directory, save_directory):
    try:
...
def update_shared_bayes_factor_csv(qmd, all_bayes_csv):
    ...
    if os.path.isfile(all_bayes_csv) is False:
        ...
        with open(all_bayes_csv, "a+") as bayes_csv:
            writer = csv.DictWriter(bayes_csv, fieldnames=fields)
            writer.writeheader()
    else:
        # print("file exists:", os.path.isfile(all_bayes_csv))
        current_csv = csv.DictReader(open(all_bayes_csv))
        current_fieldnames = current_csv.fieldnames
        new_models = list(set(fields) - set(current_fieldnames))

        if len(new_models) > 0:

            import pandas

            csv_input = pandas.read_csv(all_bayes_csv, index_col="ModelName")
...
def get_model_scores(
    ...
    for alph in list(scores.keys()):
        avg_coeff_determination[alph] = np.median(coeff_of_determination[alph])
    ...
    for mod in list(scores.keys()):
        latex_name = unique_exploration_classes[exploration_strategies[mod]].latex_name(
            mod
        )
    ...
    if collective_analysis_pickle_file is not None:
        if os.path.isfile(collective_analysis_pickle_file) is False:
            ...
            pickle.dump(model_results, open(collective_analysis_pickle_file, "wb"))
        else:
...
def plot_statistics(
    ...
    plt.clf()
    fig, top_ax = plt.subplots(figsize=(15, 2 * len(models)))

    bottom_ax = top_ax.twiny()

    top_ax.tick_params(top=True, bottom=False, labeltop=True, labelbottom=False)
    bottom_ax.tick_params(top=False, bottom=True, labeltop=False, labelbottom=True)
    ...
    bottom_ax.set_xticklabels([0, 0.5, 1], fontsize=label_size)

    top_ax.set_xlabel(
        "; ".join(top_labels), fontsize=label_size, color=top_colour, fontweight="bold"
    )
    ...
    top_ax.set_xticklabels(xticks, fontsize=label_size)
    ...
    plt.legend(custom_lines, custom_handles, bbox_to_anchor=(1.0, 0.4), fontsize=20)

    if save_to_file is not None:
        plt.savefig(save_to_file, bbox_inches="tight")


def summarise_qmla_text_file(results_csv_path, path_to_summary_file):
    ...
    with open(path_to_summary_file, "w") as summary_file:
        print(to_write, file=summary_file, flush=True)
...
def plot_scores(
    ...
    latex_model_names = [
        exploration_classes[model].latex_name(model) for model in models
    ]
    coeff_of_determination = [
    ...
    if batch_nearest_num_params_as_winners == True:
        num_true_params = len(
            qmla.model_building_utilities.get_constituent_names_from_name(true_model)
        )
        for mod in models:
            num_params = len(
    ...
    lf = lfig.LatexFigure(fraction=0.5, auto_label=False)
    ax1 = lf.new_axis()

    # ax.barh(ind, scores, width, color="blue")
    ax1.barh(ind, scores, width, color=colours)
    ax1.set_yticks(ind + width / 2)
    ax1.set_yticklabels(latex_model_names, minor=False)
    ax1.set_xlabel("Wins")
    xticks_pos = list(range(max(scores) + 1))
    ax1.set_xticks(xticks_pos, minor=False)
    custom_lines = [
    ...
    if plot_r_squared == True:
        ...
        r_sq_x_ticks = [min(coeff_of_determination), 0, 1]
    ...
    plot_title = str("Number of QMD instances won by models with $R^2$.")

    if entropy is not None:
        plot_title += str(r"\n$\mathcal{S}$=" + str(round(entropy, 2)))
    if inf_gain is not None:
        plot_title += str(r"\t $\mathcal{IG}$=" + str(round(inf_gain, 2)))
    ax1.legend(
        custom_lines,
        custom_handles,
...
def stat_metrics_histograms(champ_info, save_to_file=None):
    ...
    for plotting_data in include_plots:
        ...
        ax.set_xlim(0, 1.0)
        ax.set_title("Champions' {}".format(plotting_data["name"]))
...
def parameter_sweep_analysis(
    ...
    qmd_cumulative_results = pd.read_csv(results_csv, index_col="ConfigLatex")
    piv = pd.pivot_table(
...
def plot_evaluation_log_likelihoods(
    ...
    evaluation_plot_df = pd.DataFrame(columns=evaluation_cols)
    for i in list(combined_results.index):
        ...
        for mod in model_ids:
            if mod == instance_true_id and mod == instance_champion_id:
            ...
            ll_percentile = scipy.stats.percentileofscore(
                a=raw_lls, score=log_lls[mod], kind="weak"
            )
    ...
    all_markers = {"True + Champion": "D", "True": "X", "Champion": "D"}
    msize = 10
    marker_sizes = {"True + Champion": msize, "True": msize, "Champion": msize}
    all_colours = {
    ...
    if include_log_likelihood:
        ...
        sns.swarmplot(
            ...
            size=msize,
            hue="Classification",
            palette={c: all_colours[c] for c in unique_classifications},
        )
        ...
        if len(sub_df.instance.unique()) > 40:
            ...
            ax1.set_xticks(np.arange(0, max(sub_df.instance.unique()), 5))
        ax1.legend()
    ...
    if include_median_likelihood:
        ...
        sns.scatterplot(
            ...
            ax=ax2,
            style="Classification",
            markers={c: all_markers[c] for c in unique_classifications},
            s=msize,
            hue="Classification",
            palette={c: all_colours[c] for c in unique_classifications},
        )
    ...
    if include_champion_percentiles:
        ...
        true_df = evaluation_plot_df[(evaluation_plot_df.Classification == "True")]
        champ_df = evaluation_plot_df[(evaluation_plot_df.Classification == "Champion")]
        percentiles = np.arange(0, 101, 5)
        true_champ_perc = corresponding_percentile_frequencies(
            subset_to_inspect=true_champ_df, percentiles=percentiles
        )
        true_perc = corresponding_percentile_frequencies(
            subset_to_inspect=true_df, percentiles=percentiles
        )
        champ_perc = corresponding_percentile_frequencies(
            subset_to_inspect=champ_df, percentiles=percentiles
        )
        # horizontal bar plot
        barheight = 4
        ax.barh(
        ...
        ax.axhline(50, ls="--", color="black", label="Median", alpha=0.3)
        ax.legend()

    if save_directory is not None:
        plt.savefig(os.path.join(save_directory, "evaluation_by_likelihoods.png"))

        # save the df for manual analysis afterwards
        if save_data_as_csv:
            evaluation_plot_df.to_csv(
                os.path.join(save_directory, "data_evaluation_plot.csv")
            )
            sub_df.to_csv(os.path.join(save_directory, "data_classified_instances.csv"))
...
def corresponding_percentile_frequencies(subset_to_inspect, percentiles):
    counted_percentiles = dict(subset_to_inspect.likelihood_percentile.value_counts())
    percentile_freqs = [
        counted_percentiles[p] if p in counted_percentiles else 0 for p in percentiles
    ]
    return percentile_freqs


def count_term_occurences(combined_results, save_directory=None):
    all_constituents = []
    ...
    found_untrue_constituents = sorted(
        set(all_constituents) - set(all_true_constituents)
    )
    ...
    correct = [
        term_counter[term]["correct"] if (term in term_counter) else 0
        for term in terms_ordered_by_true_presence
    ]
    incorrect = [
        term_counter[term]["incorrect"] if (term in term_counter) else 0
        for term in terms_ordered_by_true_presence
    ]
    ...
    ax.barh(
        terms_ordered_by_true_presence, correct, color="g", left=None, label="Correctly"
    )
...
def inspect_times_on_nodes(combined_results, save_directory=None):
    ...
    if save_directory is not None:
        plt.savefig(os.path.join(save_directory, "times_v_node.png"))
