...
caption_font = {
    "fontname": "Times",
    "fontsize": str(1.8 * plot_mainfontsize),
    "weight": "bold",
}
inset_caption_font = {
    "fontname": "Times",
...
def model_pool_f_scores(
    ...
    all_models = copy.deepcopy(
        storage_instance.exploration_strategy_storage.fitness_by_f_score
    )
    all_models["model_type"] = "Candidate"

    generations = (
        storage_instance.exploration_strategy_storage.fitness_by_f_score.generation.unique()
    )
    ...
    sns.swarmplot(
        x="generation",
        y="f_score",
        data=all_models[all_models.model_type != "Champion"],
        ax=ax,
        s=marker_size,
        marker="X",
        color="blue",
    ...
    ax.tick_params(axis="both", labelsize=legend_fontsize, pad=axis_label_padding)
...
def branch_champion_dynamics(
    ...
    true_measurements = pickle.load(
        open(os.path.join(run_path, "system_measurements.p"), "rb")
    )
    storage_instance = pickle.load(
        open(os.path.join(run_path, "storage_{}.p".format(instance_id)), "rb")
    )
    storage_instance.expectation_values["time_microseconds"] = (
        storage_instance.expectation_values["time"] * 1e6
    )
    ...
    storage_instance.expectation_values["microseconds"] = (
        1e6 * storage_instance.expectation_values.time
    )
    if branches is None:
...
def term_occurences(combined_results, ax):
    all_constituents = []
    ...
    ax.legend(title="Identified:", fontsize=legend_fontsize)
...
def model_wins_and_occurences_by_f_score(all_models_generated, ax):
    bins = np.arange(0, 1.06, 0.05)

    champions = all_models_generated[all_models_generated.champion == True]
...
def nv_centre_experimental_paper_fig_3(
    ...
    if storage_instance is None:
        storage_instance = pickle.load(
            open(os.path.join(run_path, "storage_{}.p".format(focus_on_instance)), "rb")
        )
    ...
    model_pool_f_scores(storage_instance=storage_instance, ax=ax)
    ax.set_title("c", loc="left", fontdict=caption_font, pad=title_padding)
    ...
    ax.set_title("d", loc="left", fontdict=caption_font, pad=title_padding)
    ...
    model_wins_and_occurences_by_f_score(
        all_models_generated=all_models_generated, ax=ax
    )
    ax.set_title("a", loc="left", fontdict=caption_font, pad=title_padding)

    # Term occurences
    ax = fig.add_subplot(
        gs[1:3, 0],
    )
    combined_results = pd.read_csv(os.path.join(run_path, "combined_results.csv"))
    term_occurences(
        combined_results=combined_results,
        ax=ax,
    )
    ax.set_title("b", loc="left", fontdict=caption_font, pad=title_padding)
    ...
    ax.axis("off")
    ax.set_title("e", loc="left", fontdict=caption_font, pad=title_padding)
    ...
    ax.axis("off")
    ax.set_title("f", loc="left", fontdict=caption_font, pad=title_padding)
    fig.tight_layout()
