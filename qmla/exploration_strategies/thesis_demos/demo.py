...
class DemoLattice(exploration_strategy.ExplorationStrategy):
    r"""
...
class DemoIsing(DemoLattice):
    ...
    def latex_name(self, name, **kwargs):
        if "pauliLikewise" in name:
            return qmla.shared_functionality.latex_model_names.lattice_pauli_likewise_concise(
                name, **kwargs
            )
        elif "pauliSet" in name:
            return qmla.shared_functionality.latex_model_names.pauli_set_latex_name(
                name, **kwargs
            )
...
class DemoIsingFullyParameterised(DemoIsing):
    ...
    def model_from_lattice(self, lattice):
        ...
        individual_operators = [
            "pauliSet_{o}J{o}_{c}_d{N}".format(o=op, c=c, N=lattice_dimension)
            for op in self.base_terms
