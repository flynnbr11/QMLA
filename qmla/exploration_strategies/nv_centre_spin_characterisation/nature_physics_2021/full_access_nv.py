...
__all__ = ["FullAccessNVCentre"]


class FullAccessNVCentre(ExplorationStrategy):
    r"""
    ...
    def __init__(self, exploration_rules, **kwargs):
        ...
        self.fraction_particles_for_bf = (
            0.1  # testing whether reduced num particles for BF can work
        )
        ...
        self.max_num_models_by_shape = {1: 0, 2: 18, "other": 0}

    def generate_models(self, model_list, spawn_step, model_dict, **kwargs):
        ...
        self.log_print(["Generating. input model list:", model_list])
        single_qubit_terms = ["xTi", "yTi", "zTi"]
        nontransverse_terms = ["xTx", "yTy", "zTz"]
        transverse_terms = ["xTy", "xTz", "yTz"]
        all_two_qubit_terms = (
            single_qubit_terms + nontransverse_terms + transverse_terms
        )
        model = model_list[0]
        ...
        if spawn_step in [1, 2]:
            ...
            while i < 3:
                ...
                if term not in present_terms:
                    new_model = model + p_str + term
                    if (
                        check_model_in_dict(new_model, model_dict) == False
