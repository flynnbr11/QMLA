...
__all__ = ["NVCentreNQubitBath"]


class NVCentreNQubitBath(ExplorationStrategy):
    r"""
    ...
    def __init__(self, exploration_rules, **kwargs):
        ...
        self.substage_layer_champions = {
            i: {} for i in range(1, self.max_num_qubits + 1)
        }
        self.substage_champions_by_stage = {
            i: [] for i in range(1, self.max_num_qubits + 1)
        }
    ...
    def _setup_true_model_4_qubit_approx(
        self,
    ):
    ...
    def _setup_true_model_2_qubit_approx(
        self,
    ):
    ...
    def greedy_add(self, model_list, num_qubits, substage):
        ...
        self.log_print(
            [
                "Greedily adding terms. N qubits = {}, substage={}".format(
                    num_qubits, substage
                )
            ]
        )
        if substage == "rotation":
            ...
            available_terms = [  # coupling electron with this qubit
                "pauliSet_1J{q}_{p}J{p}_d{N}".format(
                    q=num_qubits, p=pauli_term, N=num_qubits
                )
                for pauli_term in self.availalbe_pauli_terms
                # ['x', 'y', 'z']
            ]
        ...
        unused_terms = list(set(available_terms) - set(present_terms))
        ...
        if self.greedy_mechanism == "greedy_single_terms":
            for term in unused_terms:
                new_model_terms = list(set(present_terms + [term]))
        ...
        new_models = [
            qmla.utilities.ensure_consistent_num_qubits_pauli_set(model)
    ...
    def get_layer_champions_by_substage(self, num_qubits, substage):
        self.log_print(
            [
                "Getting layer champions for {} qubits by substage {}".format(
                    num_qubits, substage
                )
            ]
        )
    ...
    def generate_models(self, model_list, **kwargs):
        ...
        if signal is None:
            ...
            self.log_print(
                [
                    "New stage found for {} qubits: {}. Retuning to greedy add".format(
                        num_qubits, self.substage
                    )
                ]
            )
            ...
            try:
                ...
                self.log_print(
                    [
                        "No further stage found for {} qubits. Declaring stage complete".format(
                            num_qubits
                        )
                    ]
                )
            ...
            if num_qubits == self.max_num_qubits:
                ...
                self.log_print(
                    [
                        "Trying to get first substage of {} qubits. Stages: {}".format(
                ...
                self.log_print(
                    [
                        "Increasing # qubits. Now {} with substage {}".format(
                            num_qubits, self.substage
                        )
                    ]
                )
        ...
        if signal == "default":
            if top_model is not None:
                try:
                    self.substage_layer_champions[num_qubits][self.substage].append(
                        top_model
                    )
                except:
                    self.substage_layer_champions[num_qubits][self.substage] = [
                        top_model
                    ]
            new_models = self.greedy_add(
                model_list=model_list, num_qubits=num_qubits, substage=self.substage
            )
            ...
            new_models = self.get_layer_champions_by_substage(
                num_qubits=num_qubits, substage=self.substage
            )
