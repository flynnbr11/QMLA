...
class ExperimentReducedNV(nv_centre_full_access.FullAccessNVCentre):
    def __init__(self, exploration_rules, **kwargs):
        ...
        self.max_num_models_by_shape = {2: 7, "other": 0}

        self.overwrite_exploration_class_methods(**kwargs)

    def generate_models(self, model_list, **kwargs):
        # from qmla import model_generation

        new_mods = ExperimentReducedNVal_method(model_list, **kwargs)
        return new_mods


def ExperimentReducedNVal_method(
    model_list, spawn_step, model_dict, log_file, **kwargs
):
