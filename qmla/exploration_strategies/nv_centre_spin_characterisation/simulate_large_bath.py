...
class SimulatedNVCentre(nv_centre_full_access.FullAccessNVCentre):  # inherit from this
...
def spin_system_model(
    ...
    spin_terms = [
        "pauliSet_1_{op}_d{N}".format(op=op, N=num_sites) for op in core_terms
    ]
    hyperfine_terms = [
        "pauliSet_1J{k}_{op}J{op}_d{N}".format(k=k, op=op, N=num_sites)
        for k in range(2, num_sites + 1)
        for op in core_terms
    ]
    transverse_terms = [
        "pauliSet_1J{k}_{op1}J{op2}_d{N}".format(k=k, op1=op1, op2=op2, N=num_sites)
        for k in range(2, num_sites + 1)
    ...
    if include_transverse_terms:
        all_terms.extend(transverse_terms)
...
class TestSimulatedNVCentre(SimulatedNVCentre):  # inherit from this
    # Uses some of the same functionality as
    # default NV centre spin experiments/simulations
    # but uses an expectation value which traces out
    # and different model generation
    ...
    def __init__(self, exploration_rules, **kwargs):
        ...
        self.system_probes_generation_subroutine = (
            qmla.shared_functionality.probe_set_generation.manual_set_probes
        )
