...
def tensor_expansion(binary_matrix, operator):
    I = np.identity(2)
...
def liouvillian_evolve_expectation(
    ham, t, state, log_file="QMDLog.log", log_identifier="Expecation Value", **kwargs
):
    ...
    if np.trace(rho1) > 1 + ex_val_tol or np.trace(rho1) < 0 - ex_val_tol:
        print("rho1 has a trace: ", np.trace(rho1))

    if np.trace(rho2) > 1 + ex_val_tol or np.trace(rho2) < 0 - ex_val_tol:
    ...
    expectation_value_fid = np.square(
        np.trace(
            linalg.sqrtm(np.dot(linalg.sqrtm(rho2), np.dot(rho1, linalg.sqrtm(rho2))))
        )
    )
...
def plot_probe(max_num_qubits, **kwargs):
    probe_dict = {}
...
def random_qubit():
    ...
    ex_val_tol = 1e-9
    if (
        random_num_1 ** 2 + random_num_2 ** 2 < 1 - ex_val_tol
    ):  # Checks qubit is normal within allowed machine inaccuracies
...
def liouv_separable_probe_dict(  # This may be wrong depending on what scheme of vectorisation we are to use.
    max_num_qubits, num_probes, **kwargs
):
    ...
    for qq in range(num_probes):  # Iterates to create correct number of probes
        ...
        for pp in range(
            1, N + 1
        ):  # Iterates to add additional qubits to increase probe system size
            if pp == 1:
            ...
            if np.trace(mtx) > 1 + ex_val_tol or np.trace(mtx) < 0 - ex_val_tol:
...
class Lindbladian(exploration_strategy.ExplorationStrategy):
    def __init__(self, exploration_rules, **kwargs):
        ...
        self.model_constructor = (
            qmla.shared_functionality.model_constructors.LiouvillianModel
        )
        self.true_model = "HamLiouvillian_lx_1_d1+DissLiouvillian_1A_lz_1_~_1B_ls_1_d1+DissLiouvillian_2A_lx_1_d1"
        self.initial_models = [
            "HamLiouvillian_lx_1_d1+DissLiouvillian_1A_lz_1_~_1B_ls_1_d1+DissLiouvillian_2A_lx_1_d1"
        ]
