...
class ConnectedLatticeProbabilistic(connected_lattice.ConnectedLattice):
    ...
    def model_group_fitness_calculation(
        ...
        new_fitnesses = {}
        self.log_print(["Prob conn lattice group fitness calculation"])
    ...
    def rescale_fitness(
        ...
        if rescaled_max == rescaled_min:
            ...
            new_fitness = (
                ((fitness - original_min) * new_range) / old_range
            ) + rescaled_min
        return new_fitness

    def generation_compound_fitness(self, generation, model_name_ids):
        self.log_print(
            [
                "generation compound fitness function for generation {}".format(
                    generation
                )
            ]
        )
        ...
        subgen_champ_fitnesses = self.generation_fitnesses[generation][
            subgen_champ_comparison_id
        ]
        ...
        for s in subgeneration_ids:
            ...
            corresponding_subgen_champ = list(
                set(subgen_champs).intersection(set(this_subgen_models))
            )[0]
        ...
        ranked_model_list = sorted(
            weighted_fitnesses, key=weighted_fitnesses.get, reverse=True
        )
        self.log_print(
            [
