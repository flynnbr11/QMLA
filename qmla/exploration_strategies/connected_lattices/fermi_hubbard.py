...
class FermiHubbardBase(connected_lattice.ConnectedLattice):
...
class FermiHubbardProbabilistic(FermiHubbardBase):
    def __init__(self, exploration_rules, **kwargs):
        super().__init__(exploration_rules=exploration_rules, **kwargs)
        ...
        self.qhl_models = ["FHhop_1h2_down_d3+FHonsite_3_d3"]

        self.true_model_terms_params = {
            # term : true_param
            # 'FHhop_1h2_up_d2' : 1,
        }
        self.max_num_models_by_shape = {"other": 4}
    ...
    def generate_terms_from_new_site(self, **kwargs):
...
class FermiHubbardPredetermined(FermiHubbardProbabilistic):
    def __init__(self, exploration_rules, **kwargs):
        super().__init__(exploration_rules=exploration_rules, **kwargs)
        # self.true_model = 'FHhop_1h2_up_d2'
        self.tree_completed_initially = True
        ...
        self.log_print(["Predetermined models:", self.initial_models])
        self.setup_exploration_class()


def generate_new_terms_hubbard(connected_sites, num_sites, new_sites, **kwargs):
    new_terms = []
    for pair in connected_sites:
        i = pair[0]
        j = pair[1]
        for spin in ["up", "down"]:
            hopping_term = "FHhop_{}h{}_{}_d{}".format(i, j, spin, num_sites)
            new_terms.append(hopping_term)

    for site in new_sites:
        onsite_term = "FHonsite_{}_d{}".format(site, num_sites)
...
def match_dimension_hubbard(model_name, num_sites, **kwargs):
