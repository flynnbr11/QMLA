...
from qmla.shared_functionality.model_constructors import SharedParametersModel


class TestES(exploration_strategy.ExplorationStrategy):
    def __init__(self, exploration_rules, true_model=None, **kwargs):
        ...
        # self.model_constructor = SharedParametersModel
...
class ExampleBasic(exploration_strategy.ExplorationStrategy):
    def __init__(self, exploration_rules, true_model=None, **kwargs):
...
class ExampleTwoBranches(exploration_strategy.ExplorationStrategy):
    def __init__(self, exploration_rules, true_model=None, **kwargs):
        self.true_model = "pauliSet_1J2_zJz_d4+pauliSet_2J3_zJz_d4+pauliSet_3J4_zJz_d4"
        super().__init__(
            exploration_rules=exploration_rules, true_model=self.true_model, **kwargs
...
class ExampleGreedySearch(exploration_strategy.ExplorationStrategy):
    r"""
    ...
    def generate_models(self, model_list, **kwargs):
        ...
        if self.spawn_step == 0:
            ...
            new_models = greedy_add(
                current_model=previous_branch_champ, terms=self.available_terms
            )
...
class ExampleGreedySearchTiered(exploration_strategy.ExplorationStrategy):
    r"""
    ...
    def __init__(self, exploration_rules, **kwargs):
        ...
        self.term_tiers = {
            ...
            3: [
                "pauliSet_1J2J3_xJxJx_d3",
                "pauliSet_1J2J3_yJyJy_d3",
    ...
    def generate_models(self, model_list, **kwargs):
        ...
        if self.spawn_stage[-1] is None:
            ...
            previous_branch_champ = model_list[0]
            self.log_print(["Tier champ for {} is {}".format(self.tier, model_list[0])])
            ...
            if self.tier > self.max_tier:
                ...
                self.spawn_stage.append("Complete")
                self.log_print(
                    ["Terminating search; tier champions:", self.tier_champs.values()]
                )
                return list(self.tier_champs.values())
        else:
            self.log_print(["Spawn stage:", self.spawn_stage])

        new_models = greedy_add(
            current_model=previous_branch_champ, terms=self.term_tiers[self.tier]
        )
        self.log_print(["tiered search new_models=", new_models])

        if len(new_models) == 0:
            # no models left to find - get champions of branches from this tier
            new_models = self.tier_branch_champs[self.tier]
            self.log_print(["tier champions: {}".format(new_models)])
...
class ExampleSharedParamModel(TestES):
...
def greedy_add(
    ...
    term_sets = [present_terms + [t] for t in nonpresent_terms]
