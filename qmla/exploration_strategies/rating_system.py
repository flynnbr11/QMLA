...
class RatingSystem:
    ...
    def add_ranking_model(
        ...
        if model_id in self.models.keys():
            ...
            if self.reset_rating_at_each_generation:
            ...
            elif force_new_rating and initial_rating > self.models[model_id].rating:
                # move rating e.g. to align with others in batch
                print(
                    "Reassigning rating for model {} to {} points".format(
                ...
                print(
                    "NOT reassigning rating for model {}, as its current rating ({}) is higher than the proposed ({})".format(
                        model_id, self.models[model_id].rating, initial_rating
                    )
                )

            return
        if self.reset_rating_at_each_generation or initial_rating is None:
    ...
    def get_ratings(self, model_list=None):
        for m in model_list:
            if m not in self.models:
                self.add_ranking_model(model_id=m)

        self.ratings = {m: self.models[m].rating for m in self.models}
        if model_list is None:
            return self.ratings
        else:
            model_list_ratings = {m: self.ratings[m] for m in model_list}
    ...
    def batch_update(
        ...
        models = list(
            set(qmla.utilities.flatten(list(model_pairs_bayes_factors.keys())))
        )
        models_already_present = list(set(models).intersection(self.models))
        ...
        for model_id in models:
            ...
            self.all_ratings = self.all_ratings.append(latest, ignore_index=True)
    ...
    def plot_rating_progress_single_model(self, **kwargs):
        self.plot_rating_progress_single_model_static(
            ratings_df=self.ratings_df, **kwargs
        )
...
class ELORating(RatingSystem):
    ...
    def compute_new_ratings(
        self, model_a_id, model_b_id, bayes_factor, winner_id=None, **kwargs
    ):
        ...
        model_a.update_rating(
            opponent_id=model_b_id, winner_id=winner_id, new_rating=rating_a_new
        )
        model_b.update_rating(
            opponent_id=model_a_id, winner_id=winner_id, new_rating=rating_b_new
        )
...
class ModifiedEloRating(ELORating):
    ...
    def compute_new_ratings(
        ...
        **kwargs
        ...
        print(
            "Rating update. A/B={}/{} \t BF={}".format(
                model_a_id, model_b_id, bayes_factor
            )
        )
        ...
        this_round = pd.Series(
            {
                ...
                "idx": len(
                    self.ratings_df
                ),  # to track the order in which these ratings are recorded
            }
        )
        ...
        for mod in [model_a, model_b]:
            new_idx = len(
                self.all_ratings[
                    (self.all_ratings.model_id == mod.model_id)
        ...
        self.ratings_df = self.ratings_df.append(this_round, ignore_index=True)
