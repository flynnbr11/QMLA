...
__all__ = ["FermiHubbardLatticeSet", "HubbardReducedLatticeSet"]


class FermiHubbardLatticeSet(fixed_lattice_set.LatticeSet):
    def __init__(self, exploration_rules, **kwargs):
        ...
        self.model_constructor = (
            qmla.shared_functionality.model_constructors.FermilibModel
        )
    ...
    def model_from_lattice(self, lattice):
        ...
        individual_operators = [
            "FH-hopping-sum_{spin}_{connections}_d{N}".format(
                spin=s, connections=conn_string, N=lattice_dimension
            )
            for s in ["up", "down"]
        ]

        if self.onsite_terms_present:
            site_string = "_".join([str(i) for i in range(1, 1 + lattice_dimension)])
            onsite_sum = "FH-onsite-sum_{sites}_d{N}".format(
                sites=site_string, N=lattice_dimension
            )
...
class HubbardReducedLatticeSet(FermiHubbardLatticeSet):
    def __init__(self, exploration_rules, **kwargs):
        ...
        self.max_num_models_by_shape = {4: 2, 6: 2, "other": 0}  # test
