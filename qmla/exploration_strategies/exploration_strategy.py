    ...
    * :meth:`~qmla.exploration_strategies.ExplorationStrategy._setup_logistics`
    ...
    def _setup_modular_subroutines(self):
        ...
        * :meth:`~qmla.exploration_strategies.ExplorationStrategy.latex_name` : ``latex_string_map_subroutine``
        ...
        self.qinfer_model_subroutine = (
            qmla.shared_functionality.qinfer_model_interface.QInferModelQMLA
        )

        # Prior distribution
        self.prior_distribution_subroutine = (
            qmla.shared_functionality.prior_distributions.gaussian_prior
        )
    ...
    def _setup_model_learning(self):
        ...
        *Plotting*
        ...
        self.fraction_particles_for_bf = (
            1.0  # testing whether reduced num particles for BF can work
        )
    ...
    def _setup_logistics(self):
        ...
        *Timing*
        ...
        self.max_num_models_by_shape = {1: 0, 2: 1, "other": 0}
    ...
    def true_model_terms(self):
        ...
        latex_true_terms = [self.latex_name(term) for term in true_terms]
    ...
    def get_true_parameters(
        ...
        if self.shared_true_parameters:
            ...
            true_config = pickle.load(open(self.true_params_path, "rb"))
        ...
        self.log_print(["True params dict:", self.true_params_dict])
    ...
    def generate_true_parameters(self):
        ...
        latex_terms = [
            true_model_constructor.latex_name_method(name=term) for term in terms
        ]
        ...
        self.log_print(["Generating true params; true_param_info:", true_param_info])
    ...
    def get_measurements_by_time(self):
        ...
        try:
            true_info = pickle.load(open(self.true_params_path, "rb"))
        except:
            print("Failed to load true params from path", self.true_params_path)
            raise

        plot_probes = pickle.load(open(self.plot_probes_path, "rb"))
        ...
        if self.plot_time_increment is not None:
            raw_times = list(
                np.arange(plot_lower_time, plot_upper_time, self.plot_time_increment)
            )
            self.log_print(
                [
                    "Getting plot times from plot time increment. Raw times:",
                    raw_times,
                    "lower={}; upper={}; incr={}".format(
                        plot_lower_time, plot_upper_time, self.plot_time_increment
                    ),
                ]
            )
        else:
            ...
            raw_times = list(
                np.linspace(0, plot_upper_time, num_datapoints_to_plot + 1)
            )
        ...
        self.measurements = {
            t: self.get_expectation_value(ham=self.true_hamiltonian, t=t, state=probe)
            for t in plot_times
        }
    ...
    # Measurement
    def get_expectation_value(self, **kwargs):
        r"""
        ...
        try:
            ev = self.expectation_value_subroutine(**kwargs)
        ...
        if ev < 0:
            self.log_print(["Negative likelihood : {}".format(likelihood)])
            raise ValueError("Negative likelihood; terminating")
        elif ev > 1.001:
            self.log_print(["Likelihood > 1: {}".format(likelihood)])
    ...
    # Probe states
    def generate_probes(
        self, probe_maximum_number_qubits=None, store_probes=True, **kwargs
    ):
        r"""
        ...
        self.log_print(
            [
                ...
                "probe max num qubits:",
                probe_maximum_number_qubits,
            ]
        )
        ...
        new_probes = self.system_probes_generation_subroutine(
            max_num_qubits=probe_maximum_number_qubits, **kwargs
        )
        ...
        if store_probes:
            ...
            if self.shared_probes:
                ...
                self.log_print(
                    [
                        "Using system probes as simulator probes. len keys = {}".format(
                            len(keys)
                        )
                    ]
                )
    ...
    def generate_plot_probes(self, probe_maximum_number_qubits=None, **kwargs):
        r"""
        ...
        plot_probe_dict = self.plot_probes_generation_subroutine(
            max_num_qubits=probe_maximum_number_qubits, num_probes=1, **kwargs
        )
    ...
    def get_heuristic(self, **kwargs):
        ...
        return self.model_heuristic_subroutine(**kwargs)

    # QInfer interface
    def get_qinfer_model(self, **kwargs):
        r"""
    ...
    def gaussian_prior(self, model_name, default_sigma=None, **kwargs):
        """
    ...
    def generate_evaluation_data(
        ...
        eval_data = {"probes": probes, "experiments": experiments}
        ...
        ax.set_xlabel("Time")
        fig_path = os.path.join(eval_directory, "times")
        figure_format = "pdf"
        ...
        qmla.utilities.plot_probes_on_bloch_sphere(
            ...
            save_to_file=os.path.join(
                eval_directory, "probes.{}".format(figure_format)
            ),
        )
    ...
    def plot_dynamics_of_true_model(self, probe_dict, times):
        ...
        probe_ids = [
            t for t in list(eval_probes.keys()) if t[1] == true_model_num_qubits
        ]

        for pid in probe_ids:
            probe = probe_dict[pid]

    def get_evaluation_prior(self, model_name, estimated_params, cov_mt, **kwargs):
    ...
    # Map model name strings to latex representation
    def latex_name(self, name, **kwargs):
        r"""
    ...
    def name_branch_map(self, latex_mapping_file, **kwargs):
        ...
        return qmla.shared_functionality.branch_mapping.branch_computed_from_qubit_and_param_count(
            latex_mapping_file=latex_mapping_file, **kwargs
        )
    ...
    def tree_pruning(
        ...
        if prune_step == 1:
            ...
            for branch in self.tree.branches.values():
                ...
                self.log_print(["Getting child/parents for branch", branch.branch_id])
                try:
                    ...
                    self.log_print(["Branch has no parent:", branch.branch_id])
                    pass
            ...
            for id_1, id_2 in pruned_branch.pairs_to_compare:
                self.log_print(["prune pair: id_1={}; id_2={}".format(id_1, id_2)])
                # id_1 = pair[0]
                # id_2 = pair[1]
                mod_1 = pruned_branch.model_storage_instances[id_1]
                self.log_print(["prune mod_1: {}".format(mod_1)])
                try:
                    ...
                    self.log_print(
                        [
                            "couldnt find bf {}/{}. mod_{} BF:".format(
                                id_1, id_2, id_1, mod_1.model_bayes_factors
                            )
                        ]
                    )
            ...
            models_to_keep = list(  # by ID
                set(prev_branch_models) - set(models_to_prune)
            )
            pruning_models = [  # by name
                pruned_branch.models_by_id[m] for m in models_to_keep
            ]
            pruning_sets = list(itertools.combinations(pruning_models, 2))  # by name
            self.prune_complete = True

        self.log_print(
            [
                "Prune step {}. pruning models: {} \n pruning sets: {}".format(
                    prune_step, pruning_models, pruning_sets
                )
            ]
        )
        ...
        self.log_print(
            [
                "Nominating champion as champion of final branch {}: {}".format(
                    final_branch.branch_id, final_branch.champion_name
                )
            ]
        )
    ...
    def exploration_strategy_specific_plots(
        ...
        **kwargs
        ...
        self.log_print(["Plotting methods:", self.plot_methods_by_level])
    ...
    def store_exploration_strategy_configuration(
        self, path_to_pickle_config=None, **kwargs
    ):
        dict_for_storage = self.__dict__
        if path_to_pickle_config is not None:
            pickle.dump(dict_for_storage, open(path_to_pickle_config, "wb"))
        return dict_for_storage

    def overwrite_exploration_class_methods(self, **kwargs):
        # print("[ExplorationStrategy] overwrite_exploration_class_methods. kwargs", kwargs)
        kw = list(kwargs.keys())

        attributes = ["probe_generator"]
