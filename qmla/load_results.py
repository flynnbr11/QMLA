...
def load_results(
    ...
    try:
        result_data['evaluation_data'] = pickle.load(open(os.path.join(results_dir, 'evaluation_data.p' ), 'rb'))
        result_data['storage'] = pickle.load(open(os.path.join(results_dir, 'storage_{}.p'.format(instance_id)), 'rb'))
        result_data['system_probes'] = pickle.load(open(
    ...
    try:
        ...
        result_data['champ_info'] = pickle.load(open(os.path.join(results_dir, 'champion_models',  'champions_info.p' ), 'rb'))
        result_data['bayes_factors'] = pd.read_csv(os.path.join(combined_datasets,  'bayes_factors.csv' ))
        result_data['fitness_df'] = pd.read_csv(os.path.join(combined_datasets,  'fitness_df.csv' ))
        result_data['combined_results'] = pd.read_csv(os.path.join(results_dir, 'combined_results.csv'))
