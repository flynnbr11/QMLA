...
def pauli_set_latex_name(name, **kwargs):
    r"""
...
def nv_spin_interaction(name, **kwargs):
    term = name
    ...
    latex_name += str(r")^{\otimes" + str(num_qubits) + "}")

    return "$" + latex_name + "$"


def grouped_pauli_terms(name, **kwargs):
    separate_terms = name.split("+")
    all_connections = []
    latex_term = ""
    connections_terms = {}
    for term in separate_terms:
        ...
        for c in list(sorted(connections_terms.keys())):
            connection_string = str(
                "\sigma_{" + str(c) + "}^{" + str(",".join(connections_terms[c])) + "}"
            )
...
def lattice_set_grouped_pauli(name, **kwargs):
    ...
    for term in separate_terms:
        ...
        # limits for sum
        lower_limit = str("i \in " + ",".join(this_term_connections))
        operator_string = str("\sigma_{ i }^{" + str(operator) + "}")
        if n_sites == 1:
            sites_not_present = list(
                set([int(i) for i in this_term_connections]) - set(range(1, dim + 1))
            )
            ...
            nns = set(nns)
            sites_not_present = list(set(this_term_connections) - nns)
            if len(sites_not_present) == 0:
                lower_limit = "i"
                operator_string = str(
                    "\hat{\sigma}_{i}^{"
...
def lattice_pauli_likewise_concise(name, **kwargs):
    ...
    for term in separate_terms:
        ...
        # limits for sum
        lower_limit = str("k \in " + ",".join(this_term_connections))
        if n_sites == 1:
            ...
            operator_string = str(
                "\sigma_{ \langle k, l \\rangle }^{" + str(operator) + "}"
            )
            nns = [(str(n), str(n + 1)) for n in range(1, dim)]
            nns = [",".join(list(nn)) for nn in nns]
            nns = set(nns)
...
def fermi_hubbard_latex(name, **kwargs):
    r"""
    Get latex string for model of Hubbard type.
        ...
        if "FH-hopping-sum" in components:
            ...
            for c in components:
                if c in ["down", "up"]:
                    ...
                    connected_sites += str("({},{})".format(sites[0], sites[1]))
            ...
            new_term = str(
                "\hat{H}^{" + spin_label + "}" + "_{" + connected_sites + "}"
            )
            ...
            sites_not_present = list(set(range(1, num_sites + 1)) - set(sites))
            if len(sites_not_present) > 0:
                new_term = str("\hat{N" + "}^{" + str(num_sites) + "}_{" + sites + "}")
            else:
                new_term = str("\hat{N" + "}^{" + str(num_sites) + "}")
            all_terms.append(new_term)
