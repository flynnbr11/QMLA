...
class VolumeAdaptiveParticleGuessHeuristic(ExperimentDesignHueristic):
    ...
    def design_experiment(self, epoch_id=0, **kwargs):
        ...
        if (
            ...
            if 0 < relative_change <= expected_change:
                ...
                print(
                    "Epoch {} r={}. Increasing time factor: {}".format(
                        epoch_id, relative_change, self.time_multiplicative_factor
                    )
                )
                self.time_factor_changes["increasing"].append(epoch_id)
            elif relative_change < -0.1:
                self.time_multiplicative_factor /= (
                    self.time_factor_boost
                )  # volume increasing -> use lower times
                print(
                    "Epoch {} r={}. Decreasing time factor: {}".format(
                        epoch_id, relative_change, self.time_multiplicative_factor
                    )
                )
                ...
                print(
                    "Epoch {} r={}. Maintaining time factor: {}".format(
                        epoch_id, relative_change, self.time_multiplicative_factor
                    )
                )
        ...
        for method in self.designed_times:
            d = sp.spatial.distance.pdist(sample, metric=method)
    ...
    def plot_heuristic_attributes(self, save_to_file, **kwargs):
        ...
        if "metric_distances" in plots_to_include:
            ...
            ax.set_title(
                "Raw time chosen by distance metrics", fontsize=self._label_fontsize
            )
            ax.set_ylabel("Time", fontsize=self._label_fontsize)
            ax.set_xlabel("Epoch", fontsize=self._label_fontsize)
            ax.semilogy()
    ...
    def add_time_factor_change_points_to_ax(self, ax):
        ...
        if len(self.time_factor_changes["decreasing"]) > 0:
            ax.axvline(
                ...
                ls="--",
                color="red",
                label=r"$k \rightarrow k / {}$".format(
                    np.round(self.time_factor_boost, 2)
                ),
                alpha=0.5,
            )
        ...
        if len(self.time_factor_changes["increasing"]) > 0:
            ax.axvline(
                ...
                color="green",
                # label='increase k',
                label=r"$ k \rightarrow k \times {}$".format(
                    np.round(self.time_factor_boost, 2)
                ),
                alpha=0.5,
            )
...
class TimeList(ExperimentDesignHueristic):
    def __init__(self, **kwargs):
        ...
        delta_t = (
            2 * min_t
        )  # effectively how many iterations each time is eventually learned for
        time_list = np.arange(min_t, self.max_time_to_enforce, delta_t)
        self.log_print(["delta t for heuristic:", delta_t])
