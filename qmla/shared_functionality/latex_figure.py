...
def get_latex_rc_params(default_font_size=11, font_scale=1):
    r"""
    ...
    font_sizes = {k: int(font_sizes[k] * font_scale) for k in font_sizes}
...
class LatexFigure:
    def __init__(
        ...
        self.set_fonts()
        self.delta = (
            pad  # * self.rc_params["font.size"] # was multiplied by font size - why??
        )
        ...
        if self.use_gridspec:
            ...
            if legend_axis is not None:
                self.legend_ax = self.new_axis(
                    force_position=legend_axis, span=legend_span, auto_label=False
                )
    ...
    def _set_size(self, width_pt, fraction=1, subplots=(1, 1)):
        ...
        total_width_inches = (
            self.fig_width_pt * inches_per_pt
        )  # dictated by size of page
        total_height_inches = (total_width_inches / golden_ratio) * (
            self.n_y / self.n_x
        )
    ...
    def set_size(self, width_pt, fraction=1, subplots=(1, 1)):
        ...
        self.total_size_inches = (
            total_width * inches_per_pt,
            total_height * inches_per_pt,
        )
    ...
    def new_axis(
        ...
        if self.use_gridspec:
            ...
            if grid_x_finish > self.gridspec_layout[0]:
                ...
                print(
                    "Note: span of ax greater than available number of rows; squashing ax."
                )
            ...
            if grid_y_finish > self.gridspec_layout[1]:
                print(
                    "Note: span of ax greater than available number of cols; squashing ax."
                )

            self.ax = self.fig.add_subplot(
                self.gs[grid_x_start:grid_x_finish, grid_y_start:grid_y_finish],
                **ax_params
            )
            self.ax.span = span
        ...
        if self.ax.label:
            self.ax.text(
                ...
                fontdict={"fontsize": self.rc_params["font.size"], "weight": "bold"},
            )
    ...
    def save(self, save_to_file, file_format="pdf", **kwargs):
        r"""
        Save figure.
        ...
        self.fig.savefig(save_to_file, bbox_inches="tight", **kwargs)
