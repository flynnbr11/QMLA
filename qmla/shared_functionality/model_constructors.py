...
def log_print(
    to_print_list,
    log_file='qmd.log', 
    log_identifier='Model Constructor'
):
...
class LiouvillianModel(BaseModel):
    ...
    def lindblad_to_liouvillian(self,mtx):
        ...
        term_2 = -0.5*( np.kron( np.dot( mtx.conj().T , mtx) , np.eye(int(np.sqrt(mtx.size))) ) )
        
        term_3 = -0.5*( np.kron( np.eye(int(np.sqrt(mtx.size))) , np.dot(mtx.conj().T , mtx).T ) )
    ...
    def hamiltonian_to_liouvillian(self,mtx):
        ...
        '''
        return -1j*( np.kron( mtx,np.eye(int(np.sqrt(mtx.size)))) - np.kron( np.eye(int(np.sqrt(mtx.size))), mtx.T))
    ...
    def list_to_mtx(self,letter_list):
        ...
        '''
        op = np.zeros((2**len(letter_list[0]),2**len(letter_list[0])),dtype = np.complex128)
        for x in letter_list:
            mini_op = 1
            for y in x:
                mini_op = np.kron(mini_op,qmla.model_building_utilities.core_operator_dict[y])
            op = op + mini_op
        return op
    ...
    def construct_matrix(self,parameters,string = None):
        ...
        if 'Liouvillian' in string:
            ...
            output_mtx = np.zeros((int(4**qubits),int(4**qubits)),dtype = np.complex128)
            for i in string_terms:
                if i.split('_')[0] == 'DissLiouvillian':
                    # Dissipator Handeling
                    size = int(i.split('_')[-1][1:])
                    op = np.zeros((int(2**qubits),int(2**qubits)),dtype = np.complex128)
                    i = i.split('_')[1:-1]
                    ...
                    for j in i:
                        ...
                        for num,x in enumerate(position):
                            if 'J' in matrix_type:
                                if 'J' not in x:
                                    sys.exit('Matrix pointer is the tensor product over two sites but only one site is referenced.')
                    ...
                    for num,x in enumerate(position):
                        if 'J' in matrix_type:
                            ...
                            pointer_list[num][int(x.split('J')[1])-1] = matrix_type[-1]
                        elif 'J' in x:
                            pointer_list[num][int(x.split('J')[0])-1] = matrix_type[-1]
                            pointer_list[num][int(x.split('J')[1])-1] = matrix_type[-1]
                        else:
        ...
        if param_list != []:
            sys.exit('The number of parameters exceeds the number of parameters expected from the terms')
    ...
    def terms_matrices(self):   
        ...
        for term in self.name.split('+'):
            operators.append(self.construct_matrix(([1]*self.liouvillian_parameters(string = term)), string = term))       
        return operators
