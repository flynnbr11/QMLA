...
def dimensionalise_name_by_name_list(
        constituents, true_dim, return_operator=False):
    new_list = []
...
def ising_transverse(
    ...
    **kwargs
    ...
    # TODO before using this function, need to add a max_spawn_depth to the
    # dict above for ising_transverse. How many spawns can it support?
    single_qubit_terms = ['xTi', 'yTi', 'zTi']
    nontransverse_terms = ['xTx', 'yTy', 'zTz']
...
def deterministic_transverse_ising_nn_fixed_axis(
    ...
    **kwargs
    ...
    # for this growth rule, model_list should be of length 1, either x,y or z.
    name = model_list[0]
    models = []
...
def process_hopping_term(term):
    ...
    # size of Hamiltonian # 2^2n for n sites (2n for v,up,down,double at each
    # site)
    dim = 2**(2 * num_sites)
    vectors = [
    ...
    if len(vectors) == 1:
        ...
        hopping_mtx = np.outer(
            vectors[0], vectors[1]) + np.outer(vectors[1], vectors[0])
...
def hubbard_chain_increase_dimension_full_chain(mod):
    # if going up a dimension and want to consider all newly available hopping
    # terms one by one
    import copy
...
def generate_hopping_term(
    ...
    if not isinstance(sites_list[0], list):
        sites_list = [sites_list]
    p_str = ''
    for i in range(dim):
        p_str += 'P'
...
def increase_dimension_maintain_distinct_interactions(
    ...
    # here distinct means EVERY pair of sites corresponds to a separate
    # parameter.
...
#     transverse_axis = transverse_axis_by_generator[growth_generator]
    ...
    if isinstance(spawn_stage[-1], tuple):
        dim = spawn_stage[-1][0]
