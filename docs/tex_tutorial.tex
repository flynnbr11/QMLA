...
(i) install \ttt{redis};
(ii) create a virtual Python environment for installing :term:`QMLA` dependencies without damaging other parts of 
the user's environment;
(iii) download the :term:`QMLA` codebase from the forked Github repository;
(iv) install packages upon which :term:`QMLA` depends. 
...
Assuming the \gls{run} has completed successfully, :term:`QMLA` stores the run's results in a subdirectory
...
Next, we design a basic :term:`Exploration Strategy`, for the purpose of demonstrating how to \gls{run} the algorithm.
...
\footnotetext{Recall that a single implementation of :term:`QMLA` is called an \gls{instance},    
...
The number of \glspl{instance} for each model, i.e. their \emph{\glspl{win rate}} are given in \cref{fig:qmla_win_rates}.
...
\begin{figure}[H]
    ...
    \end{center}
    \caption[Run plots]{
        :term:`QMLA` \gls{run} plots; found within \gls{run} directory e.g. \ttt{Jan\_01/01\_23/}. 
...
User interaction with the :term:`QMLA` codebase should be achieveable primarily through the \glsentryfull{es} framework. 
...
This method allows the user to impose any logic desired in constructing models;
    it is called after the completion of every branch of the  \glsentrylong{et} on the :term:`Exploration Strategy`. 
\par 

\subsection{Greedy search}\label{sec:greedy_search}

A first non-trivial :term:`Exploration Strategy` is to build models greedily from a set of \emph{primitive} terms, $\termset = \{ \hat{t} \} $. 
...
\begin{figure}[H]
    ...
    \caption[Greedy \glsentrylong{es}]{
        ...
        \textbf{b}, \ttt{graphs\_of\_branches\_ExampleGreedySearch}: shows which models reside on each branches of the \glsentrylong{et}. 
...
\begin{figure}[H]
    ...
    \caption[Tiered greedy \glsentrylong{es}]{
        ...
        \textbf{(a)}, \ttt{composition\_of\_models}. 
        \textbf{b}, \ttt{graphs\_of\_branches\_ExampleGreedySearchTiered}: shows which models reside on each branches of the \glsentrylong{et}. 
        Models are coloured by their $\fs$, and edges represent the \gls{bf} between models. 
