...
for a in range(num_tests):
  ...
  #print('Chosen sim_params: ' + str(sim_params))
  #print('Final estimate:'+str(new_eval))
...
#  loss = eval_loss(model, new_eval, true_params)
  ...
  """  
  track_par1 = [np.mean(track_particle_locs[:, 0, step]) for step in range(n_experiments)]
  track_par2 = [np.mean(track_particle_locs[:, 1, step]) for step in range(n_experiments)]
  track_par3 = [np.mean(track_particle_locs[:, 2, step]) for step in range(n_experiments)]

  par1_filename = plot_directory+'/param_1_'+str(a)
  plt.clf()
  plt.plot(track_par1)
  plt.axhline(y=param_1, xmin=0, xmax=n_experiments, hold=None, color='black')
  plt.savefig(par1_filename) 
   
  par2_filename = plot_directory+'/param_2_'+str(a)
  plt.clf()
  plt.plot(track_par2)
  plt.axhline(y=param_2, xmin=0, xmax=n_experiments, hold=None, color='black')
  plt.savefig(par2_filename) 

  par3_filename = plot_directory+'/param_3_'+str(a)
  plt.clf()
  plt.plot(track_par1)
  plt.axhline(y=param_3, xmin=0, xmax=n_experiments, hold=None, color='black')
  plt.savefig(par3_filename) 
  """
