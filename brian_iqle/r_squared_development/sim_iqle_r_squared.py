...
class GenSimQMD_IQLE(qi.FiniteOutcomeModel):
    ...
    def r_squared(self, estimated_params, probe_state, granularity=1000):
        operator_list = self._oplist
        estimated_hamiltonian = getH(estimated_params, operator_list)
        true_hamiltonian = self._trueHam
        t_max = 1000
        num_steps = math.floor(t_max)
        step_time = t_max/granularity
        estimated_expected_values = list()
        true_expected_values = list()
        times = list()

        probe_bra = probe_state
        probe_ket = np.transpose(probe_state)


        for i in range(0, num_steps):
            t = i * step_time
            e_i_H_est_t = h.exp_ham(estimated_hamiltonian, t)
            e_i_H_true_t = h.exp_ham(true_hamiltonian,t)
            H_est_psi = e_i_H_est_t @ probe_ket
            psi_H_est_psi = probe_bra @ H_est_psi

            H_true_psi = e_i_H_true_t @ probe_ket
            psi_H_true_psi = probe_bra @ H_true_psi

            estimated_expected_values.append(abs(psi_H_est_psi)**2)
            true_expected_values.append(abs(psi_H_true_psi)**2)
            times.append(t)
        slope, intercept, r_value, p_value, std_err = sp.stats.linregress(estimated_expected_values,true_expected_values)
...
plot_directory = results_directory+'/'+exp_method+'_param_by_step_plots'
r_squared_dir = results_directory + '/r_squared_plots_'+exp_method 
...
if not os.path.exists(r_squared_dir):
    os.makedirs(r_squared_dir)
...
for a in range(num_tests):
  ...
  """ define probe to calculate r squared with"""
  probe_for_r_squared = probelist[0]
  ...
  for idx_experiment in range(n_experiments):
      ...
      estimated_params=np.array([new_eval])
      r_squared_val = model.r_squared(estimated_params=estimated_params, probe_state=probe_for_r_squared)
  ...
  r_squared_plot = r_squared_dir+'/r_squared_'+str(n_particles)+'_part_'+str(n_experiments)+'_exp_test_'+str(a)
  plt.clf()
  plt.plot(r_squared_list, marker='o')
  plt.savefig(r_squared_plot)
