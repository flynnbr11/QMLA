...
class ModelLearningClass():
    ...
    def InitialiseNewModel(self, trueoplist, modeltrueparams, simoplist, simparams, numparticles, resample_thresh=0.5,checkloss=False,gaussian=False, trotter=True, IQLE=True):
...
#             self.ProbeState = pros.choose_randomprobe(self.SimOpList, self.TrueParams)
        ...
        self.GenSimModel = gsi.GenSimQMD(oplist=self.SimOpList, modelparams=self.SimParams, true_oplist = self.TrueOpList, trueparams = self.TrueParams, probelist=self.ProbeList, probecounter = self.ProbeCounter, solver='scipy', trotter=self.trotter, IQLE=self.IQLE)    # probelist=self.TrueOpList,,
