    ...
    def __init__(
        ...
        **kwargs
        ...
        for i in range(
            ...
            self.max_num_sub_generations_per_generation[i] = len(
                possible_terms)
    ...
    def generate_models(
        ...
        **kwargs
        ...
        if self.spawn_stage[-1] == 'end_generation':
            print(
                "[spin prob] End of gen {}; making new gen".format(
                    self.generation_DAG))
            print("gen champs:", self.generation_champs[self.generation_DAG])

            # get new mod ids using current generation DAG before increasing
            # it.
            new_mod_ids = list(  # now a list of lists
                self.generation_champs[self.generation_DAG].values()
            )
            new_mod_ids = sorted(
                [i for sublist in new_mod_ids for i in sublist])
            ...
            if self.generation_DAG == self.max_num_generations:
                ...
                print(
                    "QMD complete; comparing all branch champs so far:",
                    current_champs)
            ...
            for mod_id in self.models_to_build_on[self.generation_DAG]:
                ...
                if self.spawn_stage[-1] == 'make_new_gen':
                    ...
                    print(
                        "Making new generation. increasing dimension; starting model:",
                        mod_name)
                ...
                present_terms = DataBase.get_constituent_names_from_name(
                    mod_name)
            ...
            if (
                ...
                print(
                    "Increasing Generation of DAG from {}.".format(
                        self.generation_DAG))
