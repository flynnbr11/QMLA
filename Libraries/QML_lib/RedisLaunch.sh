...
echo "Inside launch redis script; host=$host; Global server: $GLOBAL_SERVER; redis port=$REDIS_PORT"
...
then
	...
	redis_run_test=`python3 RedisGlobalCheck.py -rh=$SERVER_HOST -rp=$REDIS_PORT -rqid=$QMD_ID -g=$GLOBAL_SERVER -a='check'`
