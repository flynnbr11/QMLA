...
#            self.LayerChampions[i] = 0
        ...
        try:
            ...
            self.rq_queue = Queue(self.Q_id, connection=self.redis_conn, async=test_workers, default_timeout=3600) # TODO is this timeout sufficient for ALL QMD jobs?
    ...
    def learnModel(self, model_name, use_rq = True, blocking=False): 
        ...
        if exists:
            ...
            if self.RunParallel and use_rq: # i.e. use a job queue rather than sequentially doing it. 
                from rq import Connection, Queue, Worker
                queue = Queue(self.Q_id, connection=self.redis_conn, async=self.use_rq, default_timeout=3600) # TODO is this timeout sufficient for ALL QMD jobs?
#                queue = Queue(connection=self.redis_conn, async=self.use_rq, default_timeout=3600) # TODO is this timeout sufficient for ALL 
...
#                       print("Bayes already computed bw", a,b)
    ...
    def processRemoteBayesPair(self, a=None, b=None, pair=None, bayes_threshold=1):
        ...
        try:
            ...
            print("On bayes_factors_db for pair id", pair, "value=", bayes_factors_db.get(pair))
...
#        print("Job finished count=", job_finished_count)    
    ...
    def interBranchChampion(self, branch_list=[], just_active_models=False, global_champion=False, bayes_threshold=1):
        ...
        print("All jobs have finished while computing interbranch champion")
        branch_champions_points = {}
