...
def BuildScalar_newNparam_SameQ(singoplist_2, singopnames_2, oldNparam = 1, newNparam = None):
    ...
    scalars = np.all(np.array([[  (modelname.count(string)==2 or modelname.count(string)==0) for 
...
def Build_HF_Interact(dim, inter_qbits = [0,1], paulilst = evo.paulilist()):
    
    newoplist = []
    newnamelist = []
    for axis in range(len(paulilst)):
        for i in range(dim):
            if i in inter_qbits:
                myop = paulilst[axis]
            else:
        ...
        newnamelist.append("HF"+SETTINGS.spinnames[axis][-1]+str(inter_qbits))
    newnamelist = [''.join(e for e in singnls if e.isalnum()) for singnls in newnamelist]
...
def InitialiseDB(RootN_Qbit = [0]):
    ...
    return( pd.DataFrame({'<Name>' : [spinname+str(RootN_Qbit[0]) for spinname in SETTINGS.spinnames], 
                         'N_Qbit' : [RootN_Qbit for i in range(len(SETTINGS.spinnames))], 'N_params' : 1, 'Status' : 'Ready', 'Selected' : False, 
                         'LogL_Ext' : None, 'QML_Class' : None, 'Origin_epoch' : 0,
                        'All_Operators': list(map(lambda j: [np.kron(evo.paulilst()[j],np.eye(2))] , 
                        range(  len(evo.paulilst())  ))) 
                        })  )
                        
                        
def AddDBnewNparam_SameQ(DB, epoch, RootN_Qbit = [0], MaxParamRate = SETTINGS.MaxParamRate):
# Create parameters with additional Parameters upon the SAME qubit
    ...
    if MaxN_paramsCHK+1 <= MaxParamRate:
        ...
        extractsingoplist = [(extractsingoplist['All_Operators'][i][0]) for i in range(len(extractsingopnames)) ]
        ...
        for i in range(len(NewModels[0])):
            if NewModels[1][i] not in list(DB["<Name>"]):
                DB = DB.append( pd.DataFrame(
                    ...
                    columns=SETTINGS.mycolumns) , ignore_index=True)
...
def AddHFinteraction_NewQ(DB, epoch, dim =2, inter_qbits = [0,1]):
# Adds HF-like interaction SINGLE operators to the DB operators list
    NewModels = Build_HF_Interact(dim, inter_qbits)
...
def AddScalarComboOp_NewQ(DB, epoch, dim =2, inter_qbits = [0,1], MaxParamRate = SETTINGS.MaxParamRate):
...
# !!!TODO!!! it is enough to replace "BuildScalar_newNparam_SameQ" with "BuildAll_newNparam_SameQ" in order to have ALL the combinations as above
    ...
    N_pairwise_interact = ObsNqubit*(ObsNqubit-1)/2*SETTINGS.MaxParamRate
    ...
    # Extract the basic operators and names for the HF interactions
    ...
    extractsingoplist = (DB.loc[ [DB["N_Qbit"][i] == [0]  and DB["N_params"][i] == 1 for i in range(len(DB["N_Qbit"]))] ])
    ...
    # add to the HF operators the usual basic spin Pauli operators
    extractinteractlist.extend(extractsingoplist)
    extractinteractnames.extend([spinname+str(max(inter_qbits)) for spinname in SETTINGS.spinnames])
    print(extractinteractnames)
    ...
    if len(list(itertools.combinations(extractinteractnames, NewN_paramsCHK-1))) > 0:  
        ...
        newnqbit = []
        for i in range(len(NewModels[0])):
            myqubits  = inter_qbits if ('HF' in NewModels[1][i]) else [int(max(inter_qbits))]
            newnqbit.append(myqubits)
        
    else:
        warnings.warn("The tree exploration has reached the highest N_params for " + str(max(inter_qbits)) + " qubit")
        NewModels=[]
