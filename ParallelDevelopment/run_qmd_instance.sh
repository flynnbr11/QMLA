...
python3 Exp.py -rq=1 -op="$OP" -p=$NUM_PARTICLES -e=$NUM_EXP -bt=$NUM_BAYES -rt=$RESAMPLE_T -ra=$RESAMPLE_A -pgh=$RESAMPLE_PGH -qid=$QMD_ID -rqt=75000 -g=1 -exp=$EXP_DATA -meas=$MEAS -qhl=$QHL -fq=$FURTHER_QHL -pt=$PLOTS -pkl=$PICKLE_QMD -host=$SERVER_HOST -port=$REDIS_PORT -dir=$RESULTS_DIR -log=$QMD_LOG -cb=$BAYES_CSV -cpr=$CUSTOM_PRIOR -ds="$DATASET" -dst=$DATA_MAX_TIME -dto=$DATA_TIME_OFFSET -ggr=$GROWTH -latex=$LATEX_MAP_FILE -prior_path=$PRIOR_FILE -true_params_path=$TRUE_PARAMS_FILE -true_expec_path=$TRUE_EXPEC_PATH
