...
python3 Exp.py -mqhl=$MULTIPLE_QHL -rq=1 -op="$OP" -p=$NUM_PARTICLES -e=$NUM_EXP -bt=$NUM_BAYES -rt=$RESAMPLE_T -ra=$RESAMPLE_A -pgh=$RESAMPLE_PGH -qid=$QMD_ID -rqt=200000 -g=$GAUSSIAN -exp=$EXP_DATA -meas=$MEAS -qhl=$QHL -fq=$FURTHER_QHL -pt=$PLOTS -pkl=$PICKLE_QMD -host=$SERVER_HOST -port=$REDIS_PORT -dir=$RESULTS_DIR -log=$QMD_LOG -cb=$BAYES_CSV -cpr=$CUSTOM_PRIOR -ds="$DATASET" -dst=$DATA_MAX_TIME -dto=$DATA_TIME_OFFSET -ggr=$GROWTH $ALT_GROWTH -bintimes=$BIN_TIMES_BAYES -latex=$LATEX_MAP_FILE -prior_path=$PRIOR_FILE -true_params_path=$TRUE_PARAMS_FILE -plot_probes=$PLOT_PROBES -special_probe=$SPECIAL_PROBE -true_expec_path=$TRUE_EXPEC_PATH -pmin=$PARAM_MIN -pmax=$PARAM_MAX -pmean=$PARAM_MEAN -psigma=$PARAM_SIGMA -resource=$RESOURCE_REALLOCATION
