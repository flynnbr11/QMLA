...
def individual_plots(iqle_qle):            
    ...
    y = medians
    x = range(1,1+num_exp)
    y_lower = [( y[i] - std_dev[i]) for i in range(len(y))]
    y_upper = [ (y[i] + std_dev[i] )for i in range(len(y))]
...
plot_time = get_directory_name_by_time(just_date=True) # rather than calling at separate times and causing confusion
save_figs = True
save_data = False
...
vary_resample_thresh = True
vary_pgh_factor = False

variable_parameter = 'vary'

a_options = [0.9]
resample_threshold_options = [0.5]
pgh_options = [1.0]
...
if vary_resample_a: 
    ...
    a_options = np.arange(0.85, 0.99, 0.01)

if vary_pgh_factor:
    variable_parameter += '_pgh'
    pgh_options = np.arange(0.3, 1.5, 0.2)
